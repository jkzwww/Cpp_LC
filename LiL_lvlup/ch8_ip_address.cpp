...
bool is_valid_ip(std::string ip){
    ...
    while((endPos = ip.find(delimiter,startPos)) != std::string::npos){
        tokens.push_back(ip.substr(startPos,endPos-startPos));
        startPos = endPos + 1;
    }
    tokens.push_back(ip.substr(startPos));
...
//           ip: The string to analyze with a potential ip address.
// Returns: A boolean value. True for valid ip addresses, false otherwise.
bool ref_is_valid_ip(std::string ip){
