...
int main(){
    ...
    tm birthtm = {0,0,0,birth_d,birth_m-1,birth_y-1900};
    std::time_t birthtime = std::mktime(&birthtm);
    
    time_t now = time(0);
	tm *ltm = localtime(&now);

    std::time_t diffSeconds = std::difftime(now,birthtime);
    age = diffSeconds / (60 * 60 * 24);

    std::cout << "today: " << std::ctime(&now);
    std::cout << "birth: " << std::ctime(&birthtime);
